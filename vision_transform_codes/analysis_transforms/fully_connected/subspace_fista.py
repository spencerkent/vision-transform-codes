"""
Subspace Fast Iterative Shrinkage/Thresholding for fc sparse inference
...
def run(images, dictionary, group_assignments, sparsity_weight, num_iters,
  ...
  Compare to ./fista.py. Here, thresholding is applied to the norm of a group
  of coefficients. This is also called group LCA [2], and is an algorithm
  ...
  if initial_codes is not None:
    ...
    for g_idx in range(len(group_assignments)):
      grouped_codes_tensor_aux[:, g_idx, :len(group_assignments[g_idx])] = (
  ...
  old_grouped_codes_tensor = torch.zeros_like(
      grouped_codes_tensor_aux).copy_(grouped_codes_tensor_aux)
  stop_early = False
  t_kplusone = 1.
  iter_idx = 0
  while (iter_idx < num_iters and not stop_early):
    ...
    grouped_codes_tensor = grouped_codes_tensor_aux - stepsize * (
      torch.mm(torch.mm(grouped_codes_tensor_aux.view(
          grouped_codes_tensor_aux.size(0), -1),
        grouped_dictionary) - images, grouped_dictionary.t()).view(
          grouped_codes_tensor_aux.size()))
    ...
    grouped_codes_tensor_aux = (grouped_codes_tensor +
