...
zero_mean_patches = np.transpose(
    patch_dataset['batched_patches'], (0, 2, 1)).reshape(
        (-1, patch_dim[0]*patch_dim[1])).T
img_patch_comp_means = patch_dataset['original_patch_means']


patches_test_im_1, p_pos_im_1 = patches_from_single_image(
    test_images[:, :, 0], patch_dim)
patches_test_im_2, p_pos_im_2 = patches_from_single_image(
    test_images[:, :, 1], patch_dim)
...
code_samp_im1 = display_codes(codes_test_im_1[:, 0:10], 'Example of codes for image 1')
code_samp_im2 = display_codes(codes_test_im_2[:, 0:10], 'Example of codes for image 2')
