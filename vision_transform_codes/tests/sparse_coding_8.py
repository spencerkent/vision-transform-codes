"""
Test: Laplacian scale mixture sparse coding, topographic group hyperprior
"""
...
TOPO_SIZE = [3, 3, 3]
TOPO_NEIGHBORHOODS = [3, 3, 3]
TOPO_STRIDES = [2, 2, 2]

connectivity_mat = topo_utils.generate_LSM_topo_connectivity_matrix(
    TOPO_SIZE, TOPO_NEIGHBORHOODS, TOPO_STRIDES)
spreading = (connectivity_mat / np.sum(connectivity_mat, axis=1)[:, None]).T
neighborhood_sizes = (np.ones([1, np.prod(TOPO_SIZE)], dtype='float32') *
                      np.prod(TOPO_NEIGHBORHOODS))

coeff_topo = topo_utils.compute_coeff_topo_size(TOPO_SIZE, TOPO_STRIDES)

CODE_SIZE = coeff_topo[0] * coeff_topo[1] * coeff_topo[2]
